...
def test_computer_power_feature():
    ...
    # Add some edges with per-node computer power map
    G.add_edge('A', 'B', 
               power=1000,
    ...
    base = 6
    comp_node_A_ch = base + node_to_idx['A']
    comp_node_B_ch = base + node_to_idx['B']
    print(f"\nPer-node computer power planes:")
    print(f"Edge A->B new computer power at A (ch {comp_node_A_ch}): {global_tensor[comp_node_A_ch, 0, 1]}")
    print(f"Edge A->B new computer power at B (ch {comp_node_B_ch}): {global_tensor[comp_node_B_ch, 0, 1]}")
    print(f"Edge B->C new computer power at A (ch {comp_node_A_ch}): {global_tensor[comp_node_A_ch, 1, 2]}")
    ...
    print("- Per-node computer power planes are correctly extracted")
